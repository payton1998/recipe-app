...
public class RecipeBootstrap implements ApplicationListener<ContextRefreshedEvent>{
	private final CategoryRepository categoryRpeository;
	private final RecipeRepository recipeRepository;
	private final UnitOfMeasureRepository unitOfMeasureRepository;
	
	public RecipeBootstrap(CategoryRepository categoryRpeository, RecipeRepository recipeRepository,
			UnitOfMeasureRepository unitOfMeasureRepository) {
		this.categoryRpeository = categoryRpeository;
	...
	private List<Recipe> getRecipes(){
		...
		if(!tableSpoonUomOptional.isPresent()) {
			throw new RuntimeException("Exxpected UOM is not found");
		}
		...
		//get Categories
		Optional<Category> americanCategoryOptioanl = categoryRpeository.findByDescription("American");
		
		if(!americanCategoryOptioanl.isPresent()) {
			throw new RuntimeException("Expected Category Not Found");
		}
		
		Optional<Category> mexicanCategoryOptioanl = categoryRpeository.findByDescription("Mexican");
		
		if(!mexicanCategoryOptioanl.isPresent()) {
			throw new RuntimeException("Expected Category Not Found");
		}
		
		Category americanCategory = americanCategoryOptioanl.get();
		Category mexicanCategory = mexicanCategoryOptioanl.get();
